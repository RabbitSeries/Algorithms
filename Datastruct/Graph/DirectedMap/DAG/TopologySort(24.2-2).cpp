...
void bellman_ford_after_topology( int source ) {
    ...
    for ( int i = 0; i < sortIndex.size() - 1; i++ ) {  // <path to s> <path from s to {v | for v that s can reach}>
        ...
        for ( auto const& [nextV, edgew] : std::views::zip( graph[topIndex].nextVList, graph[topIndex].edgeWList ) ) {
            if ( dis[nextV] != _UNREACHABLE && dis[nextV] > dis[topIndex] + edgew ) {
                dis[nextV] = dis[topIndex] + edgew;
                pre[nextV].clear();
                pre[nextV].push_back( topIndex );
            } else if ( dis[nextV] != _UNREACHABLE && dis[nextV] == dis[topIndex] + edgew ) {
                pre[nextV].push_back( topIndex );
            }
        }
    }
